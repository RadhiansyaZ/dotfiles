# Dotfiles wiki

This is the source-backed operational wiki for this repository. Repository files are authoritative; pages describe their behavior and link back to their sources.

Start with the content-oriented [wiki index](index.md). Consult the chronological [wiki log](log.md) for significant maintenance and validation events.

## Operational pages

- [Architecture](architecture.md)
- [Platforms](platforms.md)
- [Setup and verification](setup-and-verification.md)
- [Unix provisioning](unix-provisioning.md)
- [Windows provisioning](windows-provisioning.md)
- [Synchronization](synchronization.md)
- [Configuration packages](configuration-packages.md)
- [Maintenance](maintenance.md)
- [Privacy and sensitive boundaries](privacy-and-sensitive-boundaries.md)
- [Pending-review queue](pending/README.md)

## Wiki records

- [Wiki index](index.md)
- [Wiki log](log.md)

## Ideation artifacts

- [RFC](../idea-artifacts/RFC.md)
- [Design](../idea-artifacts/DESIGN.md)
- [Implementation plan](../idea-artifacts/PLAN.md)
